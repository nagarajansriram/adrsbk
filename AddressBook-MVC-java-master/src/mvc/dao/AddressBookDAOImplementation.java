...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into adr values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"')";
    ...
    @Override
     public void removePerson(String name)
     {
         try
         {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
           String qry="delete from adr where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
         }
         catch (SQLException e)
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update adr set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public DefaultListModel getAllNames(NameListPanel namePanel) 
    {
         ...
         conn = DBConnection.getInstance().getConnect();
            ...
            String qry = "select * from adr order by name asc";
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            Statement stmt;
            stmt = conn.createStatement();
            String qry = "select * from adr where name = '"+selectedName+"'";
            ResultSet rs;
